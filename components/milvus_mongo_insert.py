...
def milvus_mongo_upload(texts, vectors, metadata_list, milvus_dim=384, collection_name="text_vectors"):
    ...
    Args:
        texts: List[str] - 已经展平的文本列表（必须是扁平的字符串列表）
        vectors: np.ndarray - 向量数组 (N, dim)
        metadata_list: List[dict] - 元数据列表
        milvus_dim: int - 向量维度
        collection_name: str - 集合名称
    ...
    """
    try:
        # 🎯 关键改进1：移除展平逻辑，添加严格类型检查
        if not all(isinstance(t, str) for t in texts):
            # 如果不是字符串，尝试最后一次转换并警告
            non_str_indices = [i for i, t in enumerate(texts) if not isinstance(t, str)]
            st.warning(f"⚠️ 检测到 {len(non_str_indices)} 条非字符串文本，已自动转换")
        ...
        if not (text_count == vector_count == metadata_count):
            error_msg = (
                f"❌ 数据长度不一致！\n"
                f"  📝 texts: {text_count:,} 条\n"
                f"  🎯 vectors: {vector_count:,} 条\n"
                f"  📋 metadata: {metadata_count:,} 条\n\n"
                f"这通常是由于：\n"
                f"1. process_json_data() 返回了嵌套列表\n"
                f"2. 数据预处理过程中丢失或增加了条目\n"
                f"3. 向量化过程中某些文本被过滤\n\n"
                f"请检查 data_upload_page() 中的数据处理流程"
            )
            st.error(error_msg)
        ...
        # 🎯 调试信息（可选，生产环境可注释）
        print(f"DEBUG milvus_mongo_upload 接收数据:")
        print(f"  - texts: {len(texts)} 条, 类型: {type(texts)}")
        print(f"  - vectors: {vectors.shape}")
        print(f"  - metadata: {len(metadata_list)} 条")
        print(f"  - texts前3条: {texts[:3]}")
        print(f"  - texts所有类型检查: {all(isinstance(t, str) for t in texts)}")
        ...
        # 🎯 验证插入结果
        if len(milvus_ids) != text_count:
            st.warning(
                f"⚠️ 插入数量不完全匹配:\n"
                f"  预期: {text_count:,} 条\n"
                f"  实际: {len(milvus_ids):,} 条"
            )
        ...
        st.error(f"❌ 数据上传失败: {e}")
        st.info("⚠️ 请检查以下项目:")
        st.info("1. 确认数据已正确展平（无嵌套列表）")
        st.info("2. Milvus 数据库连接正常")
        st.info("3. MongoDB 数据库连接正常")  
        st.info("4. 数据格式正确")
        st.info("5. 向量维度匹配")
        raise
