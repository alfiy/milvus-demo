...
# # 创建一个统一的“提示容器”，用于显示将要自动消失的提示
# # 这个容器在每次脚本运行时重新创建，确保状态可控
# _info_container = st.empty()

# def show_temporary_message(message: str, duration: float = 3.0, level: str = "success"):
#     """
#     在统一容器中显示一条消息，并在 duration 秒后自动清空。
#     level 可以是 "success"、"info"、"warning"、"error" 中的任意一个。
#     """
#     # 根据 level 调用相应的信息显示方法
#     if level == "info":
#         _info_container.info(message)
#     elif level == "warning":
#         _info_container.warning(message)
#     elif level == "error":
#         _info_container.error(message)
#     else:
#         _info_container.success(message)

#     # 启动后台线程，在 duration 秒后清空容器
#     def _clear_after(delay: float):
#         time.sleep(delay)
#         _clear_container()

#     threading.Thread(target=_clear_after, args=(duration,), daemon=True).start()

# def _clear_container():
#     # 清空容器内容
#     global _info_container
#     _info_container.empty()
